...
@TrinoAwsProxyTest(filters = {WithConfiguredBuckets.class, TestAuthzBeforeRewrite.Filter.class})
public class TestAuthzBeforeRewrite
{
    final S3Client s3Client;
    final S3Client storageClient;
    private final TestingS3RequestRewriter s3requestRewriteController;
            ...
            implements BuilderFilter
    {
        ...
        public TestingTrinoAwsProxyServer.Builder filter(TestingTrinoAwsProxyServer.Builder builder)
        {
            return builder.withProperty("s3-security.type", OPA_S3_SECURITY_IDENTIFIER)
                    ...
                    .addModule(binder -> binder.bind(OpaS3SecurityFacade.class).toInstance((_, _, _, _) -> SecurityResponse.FAILURE))
                    .addModule(s3RequestRewriterModule("testing", TestingS3RequestRewriter.class, binder -> {
                        newOptionalBinder(binder, TestingS3RequestRewriter.class).setBinding().to(ThrowingRewriter.class).in(Scopes.SINGLETON);
                        binder.bind(RequestRewriteUtil.SetupRequestRewrites.class).asEagerSingleton();
                    }))
    ...
    @Inject
    public TestAuthzBeforeRewrite(
            S3Client s3Client,
            @ForS3Container S3Client storageClient,
            TestingS3RequestRewriter s3RequestRewriteController,
            TestingHttpServer testingHttpServer,
            TrinoAwsProxyConfig config)
    {
        this.s3Client = requireNonNull(s3Client, "internalClient is null");
        this.storageClient = requireNonNull(storageClient, "remoteClient is null");
        this.s3requestRewriteController = requireNonNull(s3RequestRewriteController, "requestRewriteController is null");
        this.baseUri = testingHttpServer.getBaseUrl();
        this.relativePath = config.getS3Path();
    }

    @Test
    public void testAuthzBeforeRewrite()
    {
        ...
        assertEquals(0, ((ThrowingRewriter) s3requestRewriteController).getCountCalls());
